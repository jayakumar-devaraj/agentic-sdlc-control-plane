...
One row per commit, each pushed as it landed. Evidence is the command's **exit code read
...
| 8c | `.specify/`, `specs/001`, ADR-0028 | — | constitution is an index of ten articles, each pointing at an existing authority rather than restating it | ☑ |
| 9 | `tests/contract/test_repository_structure.py` | — | written last, so it asserts the built tree | ☑ |
